...
class SearchFiltersList extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if(JSON.stringify(this.props.listOfSelectedDateOfBirthList) !== JSON.stringify(nextProps.listOfSelectedDateOfBirthList)) {
      this.updateFilterList(nextProps.listOfSelectedDateOfBirthList, "dateofbirth");
      this.setState({dateOfBirthList: nextProps.listOfSelectedDateOfBirthList});
    }

    if(JSON.stringify(this.props.listOfSelectedDateOfDeathList) !== JSON.stringify(nextProps.listOfSelectedDateOfDeathList)) {
      this.updateFilterList(nextProps.listOfSelectedDateOfDeathList, "dateofdeath");
      this.setState({dateOfDeathList: nextProps.listOfSelectedDateOfDeathList});
    }
    
    if(JSON.stringify(this.props.listOfSelectedStatesList) !== JSON.stringify(nextProps.listOfSelectedStatesList)) {
  ...
  tagsInputChange(output) {
    ...
    if(stateList.length !== this.state.stateList.length) {
      this.props.actions.updateStatesList(countryList);
    }
  }
  ...
  render() {
    return (<div>
            <TagsInput
                ...
                placeholder: 'Select Checkboxes in the left panel'}}
